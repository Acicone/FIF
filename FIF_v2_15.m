function [IMF,stats] = FIF_v2_15(f,options,M)

%
%  function IMF = FIF_v2_15(f,options,M)
%
...
%
%   options    Structure, generated using function Settings_FIF_v4, containing
...
%
%   See also SETTINGS_FIF_V4, GETMASK_V1_2, PLOT_IMF_V10, MAXMINS_v3_9_1.
%
%  Please cite:
%
...
if nargin < 1,  help FIF_v2_15; return; end
if nargin < 2, options = Settings_FIF_v4; end
if nargin < 3, M = []; end
...
maxmins_pp=Maxmins_v3_9_1([f_pp f_pp(1:10)],tol,options.Derivative);% to include potential extrema at the boundaries we extend periodicaly of 10 points the signal
% we make sure that the extrema identified belong to the signal before
% pre-extention of 10 points
if isempty(maxmins_pp)
    disp('No extrema detected')
    IMF=f;
    stats=[];
    return
end
maxmins_pp=maxmins_pp(maxmins_pp<=length(f_pp));
...
while countIMFs < options.NIMFs && k_pp>=options.ExtPoints
        ...
        if countIMFs>1
            if m<=stats(countIMFs-1).logM
                if options.verbose>0
                    fprintf('Warning mask length is steady or decreasing at step %1d. ',countIMFs)
                end
    ...
    if options.verbose>0
        ...
        if options.verbose>0.5
        fprintf('\n  step #            SD             Mask length \n\n')
    end
end
    stats(countIMFs).logM=m;
    a = get_mask_v1_2(MM,m,options.verbose,tol);
    ExtendSig=1==0;
 ...
 figure
        semilogy(abs(ifftA(1:(end-1)/2)).^2,'r')
        hold on
...
figure(figExt) 
            plot(f_pp)
            hold on
            plot(maxmins_pp,f_pp(maxmins_pp),'rx')           
            title(['Extrema IMF ' num2str(countIMFs) ' step # ' num2str(inStepN) ])  
            figure(figMask)
...
figure(102)
        periodogram(ifft(ifftA))
        title(['Mask IMF = ' num2str(countIMFs)])
        pause
    end
    ...
    % We consider a running average over 10 points of the signal f_pp to remove possible
    % small high frequency oscillations leftover from the subtraction of
    % previous IMFs
    if stats(countIMFs).logM>=20
        maxmins_pp=Maxmins_v3_9_1(movmean([f_pp f_pp(1:10)],10),tol,options.Derivative);% to include potential extrema at the boundaries we extend periodicaly of 10 points the signal
        % we make sure that the extrema identified belong to the signal before
        % pre-extention of 10 points
    else
        maxmins_pp=Maxmins_v3_9_1([f_pp f_pp(1:10)],tol,options.Derivative);% to include potential extrema at the boundaries we extend periodicaly of 10 points the signal
...
maxmins_pp=maxmins_pp(maxmins_pp<=length(f_pp));
    ...
    if options.saveInter==1
        save([nameFile '_intermediate_FIF_v2_15.mat'],'IMF','f','stats','-v7.3');
    end
end %end of while
...
if options.saveEnd == 1
    save([ 'Final_' nameFile '_FIF_v2_15.mat'],'IMF','stats','-v7.3');
end
...
function a=get_mask_v1_2(y,k,verbose,tol)
%

function [IMF,logM] = FIF_v1(f,options,M)


%
%  function IMF = FIF_v1(f,options)
%
...
if nargin < 1,  help FIF_v1; return; end
...
while countIMFs < options.IF.NIMFs && k_pp>=options.IF.ExtPoints && toc<options.maxTime
    ...
    while SD>options.IF.delta && inStepN < options.IF.MaxInner
        ...
        if options.verbose>0 || options.plots>=1
            [Maxs,Mins]=Maxmins_v3_4(h,options.IF.extensionType);
            if not(isempty(Maxs)) && not(isempty(Mins))
...
%         set(figMask,'Units', 'Normalized', 'OuterPosition', [0 0 1 1]);
    ...
    if options.saveInter==1
        save([nameFile '_intermediate_FIF_v1.mat'],'IMF','f','logM','-v7.3');
    end
end %end of while
...
if options.saveEnd == 1
    save([ 'Final_' nameFile '_FIF_v1.mat'],'IMF','logM','-v7.3');
end
